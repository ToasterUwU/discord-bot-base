...
- Extension (Commmand group file) that includes commands for the Owner of the Bot. ((un/re)load extension, set a status for the Bot)
- Example Extension that shows some example things so that you don't have to look at nextcord docs to remember every single little thing.
...
1. Extensions that start with _ are not loaded on startup and are hidden from autocomplete in the Owner Extension (Thats how you disable a Extension)
2. The doc string of each Extensions extension_application_command_check is used as a text for error handling when someone is not allowed to use a Command. (Set it to something that explains what's needed to pass the check)
...
5. Owner Commands only appear in the Server(s) with their IDs in the OWNER_EXTENSION_GUILD_IDS list in the GENERAL.json config. (The standard one is my private testing server)
